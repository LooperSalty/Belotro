...
- **Multijoueur en ligne** (mode « En ligne », gratuit) : prévu via **Supabase Realtime** (broadcast + presence), salons par code, modèle **host-authoritative** (un client fait tourner le moteur et diffuse des vues expurgées ; les invités envoient leurs intentions). La couche roguelike (blindes/boutique) reste solo ; en ligne = belote « classique ». Config Supabase injectée dans le fichier (URL + clé anon publique).
